...
class testing::http_server
{

  GSubprocess* _subprocess;
...
public:
  ...
  inline http_server (guint16 port = 8000, std::string_view blob_size_ = "1MiB", std::string_view local_address_ = "")
    {
      ...
      if ((g_free (blob_size), g_free (local_address), g_free (port_string), g_free (server_script)); G_UNLIKELY (NULL != tmperr))
        throw asynclib::gio_error (tmperr);
      ...
      if (G_LIKELY (NULL == tmperr && NULL != line))
        ...
        throw asynclib::gio_error (NULL != tmperr ? tmperr : g_error_new_literal (G_IO_ERROR, G_IO_ERROR_FAILED, "http server didn't started"));
    }
};